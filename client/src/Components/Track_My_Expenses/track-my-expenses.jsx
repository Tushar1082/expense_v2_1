...
export default function TrackMyExpenses() {
    const selectedTab = "trackmyexpenses";
    ...
    const handle_expense_prof_delete = async (profile_id) => {
        try {
            set_loading(true);
            const result = await fetch(`${import.meta.env.VITE_SERVER_API}/track-my-expenses/expense-profile`, {
                method: 'DELETE',
                headers: {
                    'content-type': 'application/json',
                    "Authorization": `Bearer ${user_id}` //pass user_id as token
                },
                body: JSON.stringify({ profile_id, user_id, profile_count: exp_prof.length })
            });
            ...
            if (finalRes.failed) {
                console.log(finalRes);
                show_toast({ status: 'Fail', message: 'Failed to delete profile. Please try again.' });
    ...
    return (
        <div id="main-track-my-expenses" className="flex">
            ...
            <div className="w-[100%]">
                {/* <Navbar user_profile_image={user_data?.profile_image}/> */}

                {/* Expense Profile Section */}
                {!show_selected_exp_prof.show ? <div className="relative">
                    <ExpenseProfilePopup server_url={'track-my-expenses/expense-profile'} user_id={user_id} fetch_expense_prof_data={fetch_expense_prof_data} show_create_expense_prof={show_create_expense_prof} set_show_create_expense_prof={set_show_create_expense_prof} show_toast={show_toast} set_loading={set_loading} />
                    <Navbar
                    ...
                    <div className="flex items-center justify-between m-4">
                        ...
                        <div onClick={() => set_show_create_expense_prof(true)} className="flex w-fit cursor-pointer gap-2 bg-[#f15b42] text-white font-[500] rounded-3xl px-3 py-1.5 hover:scale-105 transition-all duration-500">
                            ...
                            <button className="cursor-pointer">Create New Profile</button>
                        </div>
                    </div>
                            ...
                            exp_prof && exp_prof.map((elm, idx) => (
                                <div key={idx} className="flex items-center bg-white justify-between rounded-[10px] p-4 py-6 hover:scale-101 transition-all cursor-pointer" style={{ boxShadow: "0px 0px 5px -2px #75889a" }}>
                                    ...
                                    <div className="flex gap-2 items-center">
                                        <button onClick={() => set_show_selected_exp_prof({ show: true, expense_prof: elm, profile_id: elm.expenses_profileId })} className="flex items-center bg-[var(--color-emerald-500)] text-white py-[6px] px-[16px] rounded-3xl cursor-pointer hover:bg-[var(--color-indigo-950)] transition-all"><ViewIcon className="me-1 h-[1.3rem]" /> View</button>
                                        <button onClick={() => handle_expense_prof_delete(elm.expenses_profileId)} className="flex items-center bg-[var(--color-red-600)] text-white py-[6px] px-[16px] rounded-3xl cursor-pointer"><TrashIcon className="me-1 h-[1.3rem]" />Delete</button>
                                    </div>
                                </div>
                            ))
                        }
                    ...
                    :
                    <Expenses server_url={'track-my-expenses/expense'} user_id={user_id} expense_prof={show_selected_exp_prof.expense_prof} profile_id={show_selected_exp_prof.profile_id} set_loading={set_loading} show_toast={show_toast} set_show_selected_exp_prof={set_show_selected_exp_prof} />
                }
            </div>
        </div>
    );
